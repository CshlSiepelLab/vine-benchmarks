...
tree.%.fa: tree.%.true.nwk
	cp ../base-gtr-gamma.mod tmp.$*.fa.mod
...
tree.%.heldout.fa: tree.%.true.nwk
	cp ../base-gtr-gamma.mod tmp.$*.heldout.mod
...
tree.%.ml.mod: tree.%.nj.nwk tree.%.fa
	$(PHAST_BIN)/phyloFit --subst-mod REV --nrates 4 --tree $^ -o tree.$*.ml
...
tree.%.mrbayes.nex: tree.%.nex
	$(BIN)/addMrbayesModelToNex --in_nexus tree.$*.nex --out_nexus tree.$*.mrbayes.nex --mcmc_length $(MRBAYES_MCMCLEN) --model GTR --ngammacat 4 \
...
tree.%.true.mod: tree.%.true.nwk tree.%.fa
	cp ../base-gtr-gamma.mod tmp.$*.true.mod
...
tree.%.nj.mod: tree.%.nj.nwk tree.%.fa
	cp ../base-gtr-gamma.mod tmp.$*.nj.mod
...
tree.%.lnl: tree.%.modlnl tree.%.varlnl tree.%.beastlnl tree.%.mrbayeslnl
...
eval.all.lnl.txt: $(LNL)
	echo -e "true\tnj\tml\tvine\tbeast\tmrbayes" > lnltmp; \
	excl=",$(EXCLUDE),"; \
	for file in $(LNL); do \
...
# extract timing info
tree.%.time: tree.%.beast.term tree.%.var-time tree.%.mrbayes.term
